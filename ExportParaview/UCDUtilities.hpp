...
{
template <typename T>
struct UCDProperty final //Serve per rappresentare una proprietà
{
    std::string Label; // Nome della proprietà (es: "marker_punti")
    std::string UnitLabel;  // Unità di misura (es: "-", "°C", ecc.)

    unsigned int Size;
    unsigned int NumComponents; // Quanti numeri per ogni elemento ( 1 se è scalare)
    const T* Data; // I dati veri e propri (es: {0, 1, 2, 1, ...})
}; //Data è un puntatore costante a T, Dopo aver creato i dati, devi dare il puntatore a Data. Non puoi modificare direttamente Data, perché è const.
   //vector<T> Data; sarebbe piu facile
